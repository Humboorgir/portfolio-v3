  ...
  ({ className, placeholder, disabled, maxLength = 140, ...props }, ref) => {
    return (
      <div className={cn("relative min-h-[120px] max-w-[670px] w-full", className)}>
        <textarea
          className="absolute flex top-0 left-0 bottom-0 right-0 rounded-md px-3 py-3 bg-inherit
        focus-visible:outline-none border disabled:cursor-not-allowed 
        disabled:opacity-50 border-3 border-white/30 transition-[border] text-foreground
         placeholder:text-transparent peer resize-none focus:border-primary"
          maxLength={maxLength}
        ...
        />
        <label
          className="absolute peer-placeholder-shown:top-2 peer-focus:!text-primary
          peer-placeholder-shown:left-2 peer-placeholder-shown:text-base
          peer-focus:!-top-1.5 peer-focus:!left-[8px] peer-focus:!text-xs 
           bg-inherit px-1 transition-all duration-200 pointer-events-none
